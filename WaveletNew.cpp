    ...
    for(size_t j = 0; j < numWavelets; j++) {
        ...
        for(size_t i = 0; i < wavelet_size; i++) {
            if(!useMorlet) {
                ...
                // rickerWavelet[REAL] = Ricker(i, scale[j], wavelet_size);
                // rickerWavelet[IMAG] = 0.;
                wavelet.push_back(rickerWavelet);
            }
            else {
                std::pair<double, double> morletPair = Morlet(i, scale[j], wavelet_size);
                // fftw_complex morlet;
                // morlet[REAL] = morletPair.first;
                // morlet[IMAG] = morletPair.second;
...
std::vector<double> WaveletNew::fftw_convolve(const std::vector<double> a, const std::vector<std::pair<double, double> > b) {
    ...
    for(uint i = 0; i < optimal_length; i++) {
        xData[i][REAL] = 0.; xData[i][IMAG] = 0.;
        xWavelet[i][REAL] = 0.; xWavelet[i][IMAG] = 0.;
    }
    ...
    for(uint i = 0; i < n_b; i++) {
        xWavelet[i][REAL] = b[i].first;
        xWavelet[i][IMAG] = b[i].second;
    }
    ...
    for(uint i = 0; i < optimal_length; i++) {
        xCombined[i][REAL] = yData[i][REAL]*yWavelet[i][REAL] - yData[i][IMAG]*yWavelet[i][IMAG];
        xCombined[i][IMAG] = yData[i][REAL]*yWavelet[i][IMAG] + yData[i][IMAG]*yWavelet[i][REAL];
    }
    ...
    for(uint i = 0; i < padded_length; i++) {
        yPadded[i][REAL] = yCombined[i][REAL]/optimal_length;
        yPadded[i][IMAG] = yCombined[i][IMAG]/optimal_length;
    }
    ...
    for(uint i = startInd; i < n_a + startInd; i++) {
        result.push_back(yPadded[i][REAL]);
    }
...
std::vector<std::vector<double> > WaveletNew::fftw_convolve_fast(const std::vector<double> a, const std::vector<std::vector<std::pair<double, double> > > b) {
    ...
    for(uint i = 0; i < optimal_length; i++) {
        ...
        for(uint j = 0; j < numWavelets; j++) {
            xWavelet[j][i][REAL] = 0.; xWavelet[j][i][IMAG] = 0.;
        }
    }
    ...
    for(uint i = 0; i < n_a; i++) {
        xData[i][REAL] = a[i];
        for(uint j = 0; j < numWavelets; j++) {
            xWavelet[j][i][REAL] = b[j][i].first;
            xWavelet[j][i][IMAG] = b[j][i].second;
        }
    }
    ...
    for(uint i = 0; i < optimal_length; i++) {
        for(uint j = 0; j < numWavelets; j++) {
            xCombined[j][i][REAL] = yData[i][REAL]*yWavelet[j][i][REAL] - yData[i][IMAG]*yWavelet[j][i][IMAG];
            xCombined[j][i][IMAG] = yData[i][REAL]*yWavelet[j][i][IMAG] + yData[i][IMAG]*yWavelet[j][i][REAL];
        }
    }
    ...
    for(uint i = 0; i < padded_length; i++) {
        for(uint j = 0; j < numWavelets; j++) {
            yPadded[j][i][REAL] = yCombined[j][i][REAL]/optimal_length;
            yPadded[j][i][IMAG] = yCombined[j][i][IMAG]/optimal_length;
        }
    }
    ...
    for(uint j = 0; j < numWavelets; j++) {
        ...
        for(uint i = startInd; i < n_a + startInd; i++) {
            resultInd.push_back(yPadded[j][i][REAL]);
        }
